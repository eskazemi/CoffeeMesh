...
def create_order(payload: CreateOrderSchema):
    with UnitOfWork() as unit_of_work:
        ...
        unit_of_work.commit()
        return_payload = order.dict()  # We access the order's dictionary representation before exiting the unit of
        # work context
...
def get_order(order_id: UUID):
    try:
        with UnitOfWork() as unit_of_work: # we use a try/except block to catch the OrderNotFound Error exception.
            repo = OrdersRepository(unit_of_work.session)
            orders_service = OrdersService(repo)
